...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    vector<char>::size_type h = grid.size();
    vector<char>::size_type w = grid[0].size();
    return vector < vector<float> > (h, vector<float>(w, 1.0/h/w));
}
...
vector< vector <float> > move(int dy, int dx, 
  ...
  float blurring) 
{
    vector<float>::size_type h = beliefs.size();
    vector<float>::size_type w = beliefs[0].size();
    vector < vector <float> > newGrid (h, vector<float>(w, 0.0));
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            newGrid[i][j] = beliefs[(i-dy+h)%h][(j-dx+w)%w];
        }
    }
    ...
    float p_miss)
{
    ...
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            newGrid[i][j] = (color == grid[i][j]) ? beliefs[i][j] * p_hit : beliefs[i][j] * p_miss;
        }
    }
